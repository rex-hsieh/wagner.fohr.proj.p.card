...
png("yoy_comp/YoY_cta_comparisons.png", width = 800, height = 600, units = 'px', res=110)
op <- par(mar=c(9,4,4,2)) 
ylim <- c(0, 1.4*max( max(together$FY18),max(together$FY17) ))
yoy_comparisons_plot <- barplot(t(together),names.arg = row.names(together),
                                horiz = FALSE,las=2, ylim = ylim, main = "YoY Comparisons for CTA in July",
...
write.csv(together, file = "yoy_comp/YoY_cta_comparisons_tables.csv")
